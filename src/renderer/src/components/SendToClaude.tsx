 ...
 * level, and nothing pasted into it can change those; a session DevLobby starts
...
export function SendToClaude({
  ...
  useEffect(() => {
    ...
    void Promise.all(
      uids.map(async (uid) => {
        ...
        const result = await window.devlobby.browser.body(paneId, entry.uid)
  ...
  const describeRouting = (): string => {
    ...
    if (targetInvocation.isClaude) {
      return 'Set up for claude, but DevLobby never pressed Enter on it — the capture goes to a file and one line names it'
    }
  ...
  const send = async (): Promise<void> => {
    ...
    try {
      ...
      if (targetId === 'new') {
        ...
        const stashed = await window.devlobby.browser.stash(text, hint)
        if (!stashed.ok) {
          actions.toast(`Could not write the capture — ${stashed.error}`, 'error')
          return
        }
        // Both end up on a command line, and two of the three shells DevLobby
        ...
        actions.addTerminal({
          repoId: where.repoId,
          cwd: where.cwd,
          startupCommand: `${command} "Read ${stashed.path} — it is a network capture from the DevLobby browser pane, and my question is at the top of it."`,
          runStartup: true,
       ...
       * sitting on the prompt. So a pane where DevLobby cannot confirm Claude
      ...
      <div className="dialog">
              ...
              <>
                {!willInline && (
                  <p className="field__hint" style={{ margin: '10px 0 0' }}>
                    ...
                    {' %APPDATA%\\DevLobby\\captures'}; the pane itself gets one line naming that file.
                  </p>
                )}
        ...
        <div className="dialog__foot">
          ...
          <button
            className="btn"
            onClick={() => {
              void window.devlobby.clipboard.write(text)
...
type Targets = {
   ...
   * an effort or a flag keeps all three when DevLobby opens a session on a
 ...
 * as one thing.
 *
 * This is the decisive test, and it is better than anything DevLobby can infer
...
type SessionClaude = {
  /**
   * DevLobby pressed Enter on a claude command here, and the shell is still
   * up.
   */
  running: boolean
 ...
 *
 * So the flag DevLobby records when it actually pressed Enter is the one that
